...
BEGIN
UPDATE Salaries SET salary=-(SELECT salary FROM deleted)+(SELECT bonusPay FROM StuffDepartment WHERE post=(SELECT id_post FROM inserted))+(SELECT salary FROM inserted)
...
BEGIN
UPDATE Salaries SET salary=-(SELECT bonusPay FROM deleted)+(SELECT salary FROM Posts WHERE id_post=(SELECT post FROM inserted))+(SELECT bonusPay FROM inserted)
