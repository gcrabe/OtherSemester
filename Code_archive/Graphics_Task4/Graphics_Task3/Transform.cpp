			 ...
			 float Wx, float Wy, float Wcx, float Wcy, t_matrix c)
{
	...
	t_matrix MoveFs;
	t_move(-Vcx, -Vcy, MoveFs);

	t_matrix Scale;
	t_scale_xy(Wx / Vx, Wy / Vy, Scale);

	t_matrix Reflect;
	t_reflection_vertical(Reflect);

	t_matrix MoveSc;
	t_move(Wcx, Wcy, MoveSc);

	t_matrix FirstTransform, SecondTransform;
	
	t_times_matrix_to_matrix(Scale, MoveFs, FirstTransform);
	t_times_matrix_to_matrix(MoveSc, Reflect, SecondTransform);

	t_times_matrix_to_matrix(SecondTransform, FirstTransform, c);
}