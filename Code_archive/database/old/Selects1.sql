...
SELECT firstName, name FROM Employees CROSS JOIN Departments;
SELECT firstName, name FROM Employees as e JOIN Departments as d ON e.id_employee=d.id_department;
SELECT firstName, name FROM Employees as e INNER JOIN Departments as d ON e.id_employee=d.id_department;
SELECT firstName, name FROM Employees as e LEFT OUTER JOIN Departments as d on e.id_employee>3;
SELECT firstName, name FROM Employees as e RIGHT OUTER JOIN Departments as d ON e.id_employee>3;
--Point 2
SELECT name, director FROM Departments WHERE EXISTS (SELECT NULL);
SELECT firstName, lastName FROM Employees WHERE Employees.id_employee 
	IN (SELECT Departments.id_department FROM Departments WHERE Departments.id_department=2);
...
--Point 3
SELECT firstName=CASE firstName 
WHEN 'Èìÿ1' THEN '1' WHEN 'Èìÿ2' THEN '2' WHEN 'Èìÿ3' THEN '3' WHEN 'Èìÿ4' THEN '4' WHEN 'Èìÿ5' THEN '5' END,lastName FROM Employees;
--Point 4
